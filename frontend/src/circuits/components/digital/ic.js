 ...
 */
draw2d.shape.analog.ic = draw2d.shape.basic.Rectangle.extend({

    NAME:"draw2d.shape.analog.ic",
    params:null,
   ...
   pinsCount:0,
    ...
    leftPortsLocator : draw2d.layout.locator.Locator.extend({
        ...
        relocate:function(index, figure){
            ...
            figure.setPosition(0, 16*index+16);
        }
    }),
    rightPortsLocator : draw2d.layout.locator.Locator.extend({
        ...
        relocate:function(index, figure){
			...
			index = index-this.getParent().pinsCount/2;
            figure.setPosition(w, 16*index+16);
        }
    }),
    ...
    init:function(width, height){
		...
		var height = (this.pinsCount/2) * 16 + 16;
        this.setDimension(80, height);
        ...
        leftLocator.setParent(this);
        
        rightLocator = new this.rightPortsLocator();
        rightLocator.setParent(this);
        
        for(var i=0; i < this.pinsCount/2; i++)
        {
			this.createPort("hybrid",leftLocator);        	
        }
        
        for(var i=0; i < this.pinsCount/2; i++)
        {
			this.createPort("hybrid",rightLocator);        	
        }
    ...
    repaint : function(attributes)
    {
		...
		for(var i=0; i < this.pinsCount/2; i++)
		{
			...
			path.push("M", box.x, " ", box.y+(i*16)+16);    // ...draw line to the right middle
			path.push("L", box.x+10, " ", box.y+(i*16)+16);    // ...draw line to the right middle
		...
		{
			//console.log("Port Line");
			path.push("M", box.x+box.w-margin, " ", box.y+(i*16)+16);    // ...draw line to the right middle
			path.push("L", box.x+box.w, " ", box.y+(i*16)+16);    // ...draw line to the right middle
		}

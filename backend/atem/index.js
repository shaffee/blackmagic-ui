...
function Device(atemIpAddress){
	...
	/* Added by shaffee mayoof 1/10/2017 */
	
	this.setKey = function(sourceID) {
		...
		const cmd = new Command('CKTp', data);
		atem.sendCommand(cmd);
	};

		
	this.setChromaProperties = function(sourceID) {
		...
		data.writeUInt8(0, 1);
		data.writeUInt8(0, 2);
		data.writeUInt8(0, 3); //unknown
		
		data.writeUInt16BE(1417, 4);
		data.writeUInt16BE(1000, 6);
		data.writeUInt16BE(671, 8);
		data.writeUInt16BE(255, 10); //narrow
		
		data.writeUInt8(0, 12);
		data.writeUInt8(0, 13);
		data.writeUInt8(0, 14);
		data.writeUInt8(0, 15);


		const cmd = new Command('CKCk', data);
		atem.sendCommand(cmd);
	};
	...
	this.setKeyFill = function(sourceID) {
		var data = new Buffer(4);
		...
		const cmd = new Command('CKeF', data);
		atem.sendCommand(cmd);
	};

	this.on('KeOn', function(d) {
		
		const enabled = d.readUInt8(2);
	...
	this.onAir = function() {
		...
		const cmd = new Command('CKOn', data);
		atem.sendCommand(cmd);
	};
	
	this.setChroma = function(sourceID) {
		...
		*/
		...
		const cmd = new Command('KeBP', data);
		atem.sendCommand(cmd);
	};
	/* End editing */
